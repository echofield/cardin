    ...
    <Card className="mt-6 overflow-hidden border-[#A4B7A9] bg-gradient-to-br from-[#1D4A39] via-[#1A4334] to-[#123025] p-0 text-[#F9FAF5]">
      <div className="border-b border-[#86A090] px-6 py-4">
        <div className="flex items-center justify-between">
          <p className="text-xs uppercase tracking-[0.14em] text-[#D5E4DA]">{eyebrowLabel}</p>
          <span className="rounded-full border border-[#9DB5A8] px-2 py-0.5 text-[10px] text-[#E6F1EB]">{statusLabel}</span>
        </div>
      ...
      {notificationLabel ? (
        <div className="border-b border-[#86A090] px-6 py-4">
          <p className="text-xs uppercase tracking-[0.14em] text-[#D5E4DA]">Notification douce</p>
          <div className="mt-3 rounded-2xl border border-[#8EA495] bg-[rgba(255,255,255,0.06)] px-4 py-3 text-sm text-[#EEF5F1]">
            "{notificationLabel}"
          </div>
          {caption ? <p className="mt-3 text-xs text-[#D5E4DA]">{caption}</p> : null}
        </div>
      ) : null}
      ...
      <div className="border-t border-[#86A090] bg-[#10281F] px-6 py-4">
        ...
        <p className="mt-2 text-center text-[11px] tracking-[0.18em] text-[#D1E3D8]">{footerLabel}</p>
      </div>
    </Card>
  )
}
